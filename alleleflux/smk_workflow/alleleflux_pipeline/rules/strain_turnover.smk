...
rule replacement_classification:
    ...
    params:
        ...
        # The voter floor, the vote rule, the tie rule and the replicate rule belong
        # to THIS step only: a mouse's own verdict does not depend on how many other
        # mice could be called, so changing any of them reruns this cheap roll-up and
        # nothing upstream of it.  Defaults mirror DEFAULT_MIN_VOTERS /
        # DEFAULT_VOTE_RULE / DEFAULT_TIE / DEFAULT_REPLICATE_RULE in the script.
        ...
        replicate_rule=config["analysis"].get("strain_turnover", {}).get("replicate_rule", "average"),
    shell:
        """
        alleleflux-replacement-classification \
            ...
            --tie {params.tie} \
            --replicate_rule {params.replicate_rule}
        """
